...
- **발표자 노트**: 각 슬라이드 5요소 노트(요지/전환/핵심/상호작용/Q&A 대비/소요 시간). `Shift+N`으로 발표자 창을 열고, 그 창의 대본·페이싱 입력으로 재사용. 커버리지와 필수 항목은 검증기가 확인한다(M9·M10)
...
7. **자기 검증** — M1~M11 체크(엔진·SVG·Mermaid·발표자 브리지·노트 커버리지·구간 라벨, Strict 모드)
