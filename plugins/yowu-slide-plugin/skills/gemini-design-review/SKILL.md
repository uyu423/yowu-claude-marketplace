...
| 보호 규칙 | No | 호출자가 지정하는 수정 금지 항목 (예: deck 엔진, 특정 폰트) |
...
예시:
- `엔진 보호: deck` → `.slide.on` 토글·`position:absolute` 슬라이드·`.deck`·HUD(`#hudBar`/`#hudMeta`)·전환 JS 변경 지침 제외
- `애니 보호: 의도된 모션` → `.rv` 등장·시퀀서·SVG 드로잉 제거 지침 제외 (타이밍/이징 보정만 허용)
