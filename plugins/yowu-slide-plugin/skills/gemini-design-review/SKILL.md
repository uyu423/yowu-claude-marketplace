...
Gemini가 반환한 개선 지침(최대 10개)을 메인 세션(Claude)이 분석하여 **Edit 도구로 CSS/HTML을 직접 수정**한다.

**기본 적용 규칙:**
- CSS 속성 변경 (color, spacing, shadow, border-radius 등)만 반영
- 구조적 HTML 변경이나 외부 라이브러리 추가 지침은 무시
- Gemini가 제안한 값이 명확하고 구체적인 것만 적용 (모호한 지침은 스킵)

**호출자 보호 규칙 적용:**
호출자가 보호 규칙을 전달한 경우, 해당 규칙에 위배되는 지침은 무시한다.
예시:
- `scroll-snap 보호` → scroll-snap 관련 CSS 변경 금지
- `폰트 보호: NanumSquareNeo` → 폰트 변경 지침 무시
...
- Gemini 응답을 그대로 복사하지 않는다 -- 반드시 각 지침을 분석하고 안전한 것만 적용
