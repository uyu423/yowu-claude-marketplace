...
HTML 작성이 완료된 후, `gemini` CLI를 활용하여 디자인 보완 지침을 받고 메인 세션에서 적용한다.
**사용자 환경에 `gemini` CLI가 설치된 경우에만 실행하며, 설치되지 않았으면 조용히 건너뛴다 (quiet pass).**

**실행 절차:**

1. **설치 확인**: Bash로 `command -v gemini` 실행. 종료 코드가 0이 아니면 이 단계를 **무조건 스킵**한다. 사용자에게 설치를 권유하거나 에러를 표시하지 않는다.

2. **리뷰 프롬프트 로드**: `references/gemini-review-prompt.md` 파일을 읽는다. 이 파일에 디자인 리뷰 기준과 출력 포맷이 정의되어 있다.

3. **Gemini 호출**: 아래 패턴으로 Bash 실행한다:
   ```bash
   cat "{html-file-path}" | gemini -p "$(cat "{skill-dir}/references/gemini-review-prompt.md")" -o text
   ```
   - HTML 전문을 stdin으로 전달하고, 리뷰 프롬프트를 `-p` 옵션으로 전달한다
   - `-o text` 옵션으로 plain text 출력을 받는다
   - 타임아웃: 120초. 타임아웃이나 에러 발생 시에도 quiet pass (기존 HTML 유지)

4. **지침 적용**: Gemini가 반환한 개선 지침(최대 10개)을 메인 세션(Claude)이 분석하여 **Edit 도구로 CSS/HTML을 직접 수정**한다.
   - 적용 기준: scroll-snap 동작을 깨뜨리지 않는 CSS 수정만 반영한다
   - 구조적 HTML 변경이나 외부 라이브러리 추가 지침은 무시한다
   - 폰트(NanumSquareNeo) 변경 지침은 무시한다
   - 적용한 개선 사항을 사용자에게 간략히 요약한다: `"Gemini 디자인 리뷰 반영: {적용 항목 수}건 적용"`

5. **스킵 시 동작**: gemini CLI 미설치, 타임아웃, 에러 등 모든 실패 케이스에서:
   - 에러 메시지를 사용자에게 보여주지 않는다
   - 기존 HTML을 그대로 유지한다
   - 다음 단계(파일 경로 안내)로 자연스럽게 진행한다
...
- `references/gemini-review-prompt.md`: Gemini CLI에 전달하는 디자인 리뷰 프롬프트. 리뷰 기준, 출력 포맷, 제약 조건이 정의되어 있다.
