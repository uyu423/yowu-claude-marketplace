...
### Step 1: 테마 선택 + Step 2: 디자인 소스 결정

콘텐츠를 파악한 뒤, **하나의 AskUserQuestion으로 테마와 디자인 스타일을 함께 물어본다.**
콘텐츠에서 테마와 디자인이 명백히 추론 가능하면(예: 코드가 포함된 기술 발표 → Dark) 질문 없이 기본값으로 진행하고, 결과물 하단에 "테마/디자인 변경을 원하시면 알려주세요"를 안내한다.
사용자가 이미 요청에서 명시한 경우(예: "다크로", "심플하게")에도 질문을 생략한다.

```
발표 내용을 분석했습니다. 아래 옵션을 선택해주세요:

테마:
1. Dark (권장) — 기술 발표에 적합한 어두운 배경
2. Light — 기획 제안/비즈니스에 적합한 밝은 배경

디자인 스타일:
1. /frontend-design (기본값) — 화려하고 창의적인 비주얼
2. 자체 디자인 가이드 (심플) — 절제된 미니멀 스타일
3. 디자인 가이드 직접 제공

엔터만 누르면 기본값(1, 1)으로 진행합니다.
```
...
| 선택       | 디자인 소스                                       | 특징                                              |
| ---------- | ------------------------------------------------- | ------------------------------------------------- |
| 1 (기본값) | frontend-design skill                             | 화려하고 창의적인 비주얼 (glassmorphism, glow 등) |
| 2          | `references/design-system.md` (자체 디자인시스템) | 일관되고 절제된 미니멀 스타일                     |
| 3          | 사용자 제공 가이드                                | 사용자 지정 스타일                                |

**frontend-design 로드 방법 (선택 1):**
...
각 섹션에 적합한 슬라이드 타입을 배정한다 (아래 카탈로그 참조).
