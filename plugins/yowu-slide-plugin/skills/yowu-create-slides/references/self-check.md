...
아래 11개 항목은 HTML에서 기계적으로 판정 가능하다. 1개 이상 실패 시 해당 슬라이드나 모듈만 Edit으로 수정 (1회 한정). 재시도 후에도 실패하면 통과 처리 + 메타 블록에 `[SELF-CHECK WARN]` 기록.
...
| M9 | 노트 커버리지 | 모든 `section.slide`에 `aside.slide-notes`가 있다 (발표자 창의 대본 소스) |
| M10 | 노트 필수 항목 | 각 노트에 `요지`·`핵심`·`소요 시간` `h4`가 있다. **`소요 시간`은 발표자 창 페이싱의 입력이라 한 장만 비어도 그 뒤 계산이 어긋난다** |
| M11 | 구간 라벨 | 슬라이드 10장 초과 덱은 모든 `.slide`에 `data-part`가 있다 (HUD가 `dataset.part`를 읽는다) |
...
Self-check: PASS (M1-M11 전부 통과) | LLM: 8/10 (80%)
또는
